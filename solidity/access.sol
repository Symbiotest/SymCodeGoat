 ...
 * @notice This contract demonstrates several common vulnerabilities in DeFi protocols
 * including access control issues, reentrancy, and price manipulation.
 */
contract VulnerableVault is ERC20, Ownable, ReentrancyGuard {
    ...
    // Insecure: No initialization protection
    constructor(address _token) ERC20("Vulnerable Vault", "vVLT") {
        token = IERC20(_token);
        _transferOwnership(msg.sender);
    }
    
    /**
     * @dev Insecure: Missing access control
     */
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        // Insecure: No share calculation protection
    ...
    /**
     * @dev Insecure: Reentrancy and price manipulation vulnerability
     */
    function withdraw(uint256 shares) external nonReentrant {
        require(shares > 0, "Cannot withdraw 0");
        require(shares <= userShares[msg.sender], "Insufficient balance");
        
        // Insecure: No flash loan protection
        uint256 amount = shares.mul(totalDeposits).div(totalSupply());
        
        // Insecure: State changes after external call
        _burn(msg.sender, shares);
        userShares[msg.sender] = userShares[msg.sender].sub(shares);
        totalDeposits = totalDeposits.sub(amount);
        
        // Insecure: No reentrancy protection on ERC20 transfer
    ...
    /**
     * @dev Insecure: Price manipulation vulnerability
     */
    function getPricePerFullShare() external view returns (uint256) {
        if (totalSupply() == 0) return 1e18;
        return totalDeposits.mul(1e18).div(totalSupply());
    }
    
    /**
     * @dev Insecure: Flash loan attack vector
     */
    function flashLoan(
        address recipient,
        uint256 amount,
        ...
        // Insecure: No fee charged for flash loans
        token.transfer(recipient, amount);
        
        // Insecure: No validation of the callback
        if (data.length > 0) {
            (bool success, ) = recipient.call(data);
            require(success, "Flash loan callback failed");
        }
        
        // Insecure: No check for profit extraction
        uint256 balanceAfter = token.balanceOf(address(this));
        require(balanceAfter >= balanceBefore, "Flash loan not repaid");
    }
    
    /**
     * @dev Insecure: Admin function with no timelock
     */
    function emergencyWithdraw(address _token) external onlyOwner {
        // Insecure: No timelock or delay
 ...
 * @notice This contract demonstrates common token vulnerabilities
 */
contract VulnerableToken is ERC20, Ownable {
    ...
    /**
     * @dev Insecure: Integer overflow vulnerability
     */
    function transfer(address to, uint256 value) public override returns (bool) {
        require(balances[msg.sender] >= value, "Insufficient balance");
        
        // Insecure: No protection against overflow
    ...
    /**
     * @dev Insecure: Approval race condition
     */
    function approve(address spender, uint256 value) public override returns (bool) {
        // Insecure: No protection against race condition
    ...
    /**
     * @dev Insecure: Missing return value check
     */
...
// Insecure: Using outdated SafeMath
library SafeMath {
